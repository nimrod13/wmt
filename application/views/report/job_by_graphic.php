...
<?php
    ...
    {
        if( isset( $bills_arr[ $batch->user_id ] ) )
        {
            $bills_arr[ $batch->user_id ] += $batch->no_bills;
        }
        else
        {
            $bills_arr[ $batch->user_id ] = $batch->no_bills;
...
<?php if( !empty( $bills_arr ) ): ?>
    <script type="text/javascript" class="code">
        ...
        var max_y = <?= max( $bills_arr ); ?>;

        var ticks = [
            <?php
                foreach( $bills_arr as $user_id => $no_bills )
                {
    ...
    // redraws the graphic after ajax. 
